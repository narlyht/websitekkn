...
<head>
    ...
    <title>Edit Berita - Dashboard Admin Cikokol</title>
    ...
    <div class="max-w-3xl mx-auto px-6">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <h1 class="font-classic text-3xl font-bold text-classic-green">Edit Berita KKN</h1>
            </div>
        </div>
        ...
        <form action="{{ route('admin.news.update', ['id' => $news->id]) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl p-8 border border-classic-border shadow-xl space-y-6">
            @csrf
            @method('PUT')
            
            <div>
                <label class="block text-xs font-bold text-classic-green uppercase tracking-wider mb-2">Judul Berita / Kegiatan</label>
                <input type="text" name="title" value="{{ old('title', $news->title) }}" required class="w-full px-4 py-2.5 rounded-xl border border-classic-border text-sm outline-none focus:ring-2 focus:ring-classic-green"/>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="block text-xs font-bold text-classic-green uppercase tracking-wider mb-2">Tanggal Berita</label>
                    <input type="text" name="date" value="{{ old('date', $news->date) }}" required class="w-full px-4 py-2.5 rounded-xl border border-classic-border text-sm outline-none focus:ring-2 focus:ring-classic-green"/>
                </div>
                ...
                <div>
                    ...
                    <select name="category" required class="w-full px-4 py-2.5 rounded-xl border border-classic-border text-sm bg-white outline-none focus:ring-2 focus:ring-classic-green">
                        @foreach(['Legalitas Usaha', 'Seminar & Workshop', 'Digital Branding', 'Pengabdian Warga'] as $cat)
            ...
            <div>
                <label class="block text-xs font-bold text-classic-green uppercase tracking-wider mb-2">Ganti Foto Cover Berita (Opsional)</label>
                ...
                @if($news->image)
                    <div class="mt-2 flex items-center gap-2">
                        <img src="{{ $news->image }}" alt="Cover" class="w-16 h-12 object-cover rounded-lg border border-classic-border"/>
                        <span class="text-xs text-classic-charcoal/60">Foto Cover Berita Saat Ini</span>
                    </div>
                @endif
            </div>
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2" required class="w-full px-4 py-2.5 rounded-xl border border-classic-border text-sm outline-none focus:ring-2 focus:ring-classic-green">{{ old('excerpt', $news->excerpt) }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-bold text-classic-green uppercase tracking-wider mb-2">Isi Berita Lengkap</label>
                <textarea name="content" rows="6" required class="w-full px-4 py-2.5 rounded-xl border border-classic-border text-sm outline-none focus:ring-2 focus:ring-classic-green">{{ old('content', $news->content) }}</textarea>
            </div>
